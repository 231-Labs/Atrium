  ...
  useEffect(() => {
    ...
    async function loadSubscribersFromEvents(spaceId: string): Promise<SubscriberAvatar[]> {
      ...
      const spaceEvents = events.data.filter((event: any) => 
        event.parsedJson?.space_id === spaceId
      );
